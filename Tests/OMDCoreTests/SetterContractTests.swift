...
final class SetterContractTests: XCTestCase {
  func testDitheringInvalidSelectorReturnsBlockedResult() throws {
    let result = try DitheringService(resolver: MissingResolver()).setDithering(
      DisplaySelector("uuid:not-a-real-display"), enabled: false)
  ...
  func testDitheringUnexpectedResolverFailurePropagates() {
    XCTAssertThrowsError(
      try DitheringService(resolver: UnexpectedResolver()).setDithering(
        DisplaySelector("bad"), enabled: false)
    ) { error in
  ...
  func testICCInvalidSelectorReturnsBlockedResultBeforeFileCheck() throws {
    let result = try ICCProfileService(
      resolver: MissingResolver(), backend: FakeICCProfileBackend()
    ).setICCProfile(
      DisplaySelector("uuid:not-a-real-display"),
      profileURL: URL(fileURLWithPath: "/definitely/not/readable.icc")
    )
  ...
  func testICCUnexpectedResolverFailurePropagates() {
    XCTAssertThrowsError(
      try ICCProfileService(resolver: UnexpectedResolver(), backend: FakeICCProfileBackend())
        .setICCProfile(
          DisplaySelector("bad"),
          profileURL: URL(fileURLWithPath: "/tmp/profile.icc")
        )
    ) { error in
      XCTAssertEqual(error as? DisplayControlError, .unexpected("CG failure"))
    }
