...
final class DitheringServiceTests: XCTestCase {
  func testReadDitheringReturnsActiveExternalFramebufferState() {
    let backend = FakeDitheringBackend(framebuffers: [
      framebuffer(id: 1, external: true, active: true, enabled: false)
    ])
    let service = DitheringService(
      resolver: FakeDisplayResolver(isBuiltin: false), backend: backend)
  ...
  func testSetDitheringUsesBuiltinDisplayInternalFramebuffer() throws {
    let backend = FakeDitheringBackend(framebuffers: [
      ...
      framebuffer(id: 2, external: false, active: true, enabled: true),
    ])
  ...
  private static func framebuffer(id: UInt64, external: Bool, active: Bool, enabled: Bool?)
    -> DitheringFramebuffer
  {
    DitheringFramebuffer(
      ...
      isExternal: external,
      isActive: active,
      enableDither: enabled
    )
  }

  private func framebuffer(id: UInt64, external: Bool, active: Bool, enabled: Bool?)
    -> DitheringFramebuffer
  {
...
private struct FakeDisplayResolver: DisplayResolving {
  ...
  func resolve(_ selector: DisplaySelector) throws -> ResolvedDisplay {
    Self.resolvedDisplay(isBuiltin: isBuiltin)
  }
}
...
private final class FakeDitheringBackend: DitheringBackend, @unchecked Sendable {
  ...
  init(
    framebuffers: [DitheringFramebuffer],
    setResult: Bool = true,
    updateReadbackOnSet: Bool = true
  ) {
  ...
  func setDithering(_ enabled: Bool, on registryID: UInt64) -> Bool {
    ...
    if setResult, updateReadbackOnSet,
      let index = storedFramebuffers.firstIndex(where: { $0.registryID == registryID })
    {
