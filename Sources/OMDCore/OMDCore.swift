...
public func listDisplays() throws -> [DisplayTarget] {
  try DisplayStateReader().listDisplays()
}

public func readDisplayState(_ display: DisplaySelector) throws -> DisplayState {
  try DisplayStateReader().readDisplayState(display)
}

public func listResolutionModes(_ display: DisplaySelector) throws
  -> DisplayListResult<ResolutionMode>
{
  try ResolutionModeService().listResolutionModes(display)
}

public func setResolutionMode(
...
public func listDisplayModes(_ display: DisplaySelector) throws
  -> DisplayListResult<DisplayMode>
{
  try DisplayModeService().listDisplayModes(display)
}

public func setDisplayMode(
