...
struct ICCProfileService: Sendable {
  ...
  func readICCProfile(_ display: ResolvedDisplay) -> DisplayAxis<URL> {
    guard let deviceID = backend.deviceID(for: display.displayID) else { return .unreadable(source: "ColorSync display UUID unavailable") }

    if let profile = backend.profile(for: deviceID) { return .readable(profile.url, source: profile.source) }

    return .unreadable(source: "ColorSync profile URL unavailable")
  }

  func listICCProfiles() throws -> [ICCProfile] { uniqueSortedProfiles(try backend.installedProfiles()) }

  func listDisplayAssignableICCProfiles() throws -> [ICCProfile] { uniqueSortedProfiles(try backend.installedDisplayProfiles()) }
  ...
  private func uniqueSortedProfiles(_ profiles: [ICCProfile]) -> [ICCProfile] {
    ...
    for profile in profiles {
      ...
      if profilesByPath[path] == nil { profilesByPath[path] = profile }
    }
    return profilesByPath.values.sorted { lhs, rhs in
      let nameOrder = lhs.name.localizedStandardCompare(rhs.name)
      if nameOrder != .orderedSame { return nameOrder == .orderedAscending }
      return ICCProfileIdentity.sortKey(lhs.url).localizedStandardCompare(ICCProfileIdentity.sortKey(rhs.url)) == .orderedAscending
    }
  }
  ...
  func setICCProfile(_ selector: DisplaySelector, profileURL: URL) throws -> DisplaySetResult {
    ...
    for attempt in 0..<10 {
      if let readbackURL = backend.profile(for: deviceID)?.url, ICCProfileIdentity.sameFile(readbackURL, profileURL) { return .applied("ICC profile applied") }
      if attempt < 9 { backend.waitBeforeReadback() }
    }
...
struct ICCDisplayDeviceID: @unchecked Sendable { var rawValue: CFUUID }
...
struct LiveICCProfileBackend: ICCProfileBackend {
  ...
  func installedDisplayProfiles() throws -> [ICCProfile] { try installedProfiles(parse: Self.installedDisplayProfile(from:)) }

  private func installedProfiles(parse: @escaping (CFDictionary) -> ICCProfile?) throws -> [ICCProfile] {
    let collector = ICCProfileCollector(parse: parse)
    var seed: UInt32 = 0
    var error: Unmanaged<CFError>?
    let options = [kColorSyncWaitForCacheReply.takeUnretainedValue() as String: kCFBooleanTrue as Any] as CFDictionary

    ColorSyncIterateInstalledProfilesWithOptions(
      { profileInfo, userInfo in
        guard let profileInfo, let userInfo else { return true }
        let collector = Unmanaged<ICCProfileCollector>.fromOpaque(userInfo).takeUnretainedValue()
        if let profile = collector.parse(profileInfo) { collector.profiles.append(profile) }
        return true
      }, &seed, Unmanaged.passUnretained(collector).toOpaque(), options, &error)

    if let error { throw DisplayControlError.unexpected("ColorSync profile iteration failed: \(error.takeRetainedValue())") }
  ...
  func profile(for deviceID: ICCDisplayDeviceID) -> ICCProfileReadback? {
    guard let info = ColorSyncDeviceCopyDeviceInfo(kColorSyncDisplayDeviceClass.takeUnretainedValue(), deviceID.rawValue)?.takeRetainedValue() as? [String: Any]
    else { return nil }
  ...
  func setCustomProfile(_ profileURL: URL, for deviceID: ICCDisplayDeviceID) -> Bool {
    ...
    return ColorSyncDeviceSetCustomProfiles(kColorSyncDisplayDeviceClass.takeUnretainedValue(), deviceID.rawValue, profiles)
  }

  func waitBeforeReadback() { Thread.sleep(forTimeInterval: 0.1) }

  static func profileReadback(from info: [String: Any]) -> ICCProfileReadback? {
    if let url = customProfileURL(from: info) { return ICCProfileReadback(url: url, source: "ColorSync custom profile") }
    if let url = factoryProfileURL(from: info) { return ICCProfileReadback(url: url, source: "ColorSync factory profile") }
    return nil
  }
  ...
  static func installedProfile(from info: CFDictionary) -> ICCProfile? {
    ...
    guard let url = profileURLValue(dictionary[urlKey]) else { return nil }

    let descriptionKey = kColorSyncProfileDescription.takeUnretainedValue()
    let rawName = (dictionary[descriptionKey] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines)
    let name = rawName.flatMap { $0.isEmpty ? nil : $0 } ?? url.deletingPathExtension().lastPathComponent
    return ICCProfile(name: name, url: url)
  }
  ...
  static func installedDisplayProfile(from info: CFDictionary) -> ICCProfile? {
    ...
    var error: Unmanaged<CFError>?
    guard let colorSyncProfile = ColorSyncProfileCreateWithURL(profile.url as CFURL, &error) else { return nil }
  ...
  static func customProfileURL(from info: [String: Any]) -> URL? {
    ...
    guard let section = info[customProfilesKey] as? [String: Any] else { return nil }

    let defaultKey = kColorSyncDeviceDefaultProfileID.takeUnretainedValue() as String
    if let url = profileURLValue(section[defaultKey]) { return url }

    if let factoryDefaultID = factoryDefaultProfileID(from: info) { return profileURLValue(section[factoryDefaultID]) }

    let urls = section.compactMap { key, value in key == defaultKey ? nil : profileURLValue(value) }
  ...
  static func factoryProfileURL(from info: [String: Any]) -> URL? {
    ...
    guard let section = info[factoryProfilesKey] as? [String: Any] else { return nil }

    guard let defaultProfileID = factoryDefaultProfileID(from: info, factoryProfiles: section), let profileInfo = section[defaultProfileID] as? [String: Any]
    else { return nil }

    let profileURLKey = kColorSyncDeviceProfileURL.takeUnretainedValue() as String
    return profileURLValue(profileInfo[profileURLKey]) ?? profileURLValue(profileInfo["DeviceProfileURL"])
  }

  private static func factoryDefaultProfileID(from info: [String: Any]) -> String? {
    let factoryProfilesKey = kColorSyncFactoryProfiles.takeUnretainedValue() as String
    guard let factoryProfiles = info[factoryProfilesKey] as? [String: Any] else { return nil }
    return factoryDefaultProfileID(from: info, factoryProfiles: factoryProfiles)
  }

  private static func factoryDefaultProfileID(from info: [String: Any], factoryProfiles: [String: Any]) -> String? {
    let defaultKey = kColorSyncDeviceDefaultProfileID.takeUnretainedValue() as String
    if let defaultProfileID = factoryProfiles[defaultKey] as? String { return defaultProfileID }
...
private final class ICCProfileCollector {
  ...
  init(parse: @escaping (CFDictionary) -> ICCProfile?) { self.parse = parse }
}
