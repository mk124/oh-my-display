...
package enum CurrentOffUpdate: Equatable, Sendable {
  ...
  package var message: String {
    switch self {
    case .succeeded:
      return "Current was turned Off; this display is no longer controlled by a profile."
    case .failed(let summary):
      return "Current Off update also failed: \(summary). Profile control state could not be cleared."
    }
  }
}
...
extension OMDAppCore {
  package func recoverDirectMutation(
    ...
    guard canRestore else {
      return .restoreUnavailable(unavailableReason, turnCurrentOff(for: display))
    }
    ...
    do {
      ...
      return result.succeeded
        ? .restored(result)
        : .restoreFailed(result, turnCurrentOff(for: display))
    } catch {
      return .restoreUnavailable("restore threw: \(error)", turnCurrentOff(for: display))
    }
  }
