...
extension OMDAppCore {
  package func reconcile(trigger: DisplayEventTrigger) throws -> [DisplayReconcileResult] {
    ...
    for display in displays {
      guard let recordIndex = recordIndex(for: display.selector) else {
        results.append(
          DisplayReconcileResult(
            display: display,
            outcome: .skipped(reason: .off, profileID: nil)))
        continue
      }
      ...
      guard document.displays[recordIndex].binding.isStrong else {
        results.append(
          DisplayReconcileResult(
            display: display,
            outcome: .skipped(reason: .weakBinding, profileID: nil)))
        continue
      }
      guard let profile = document.displays[recordIndex].profiles.first(where: {
        ...
        results.append(
          DisplayReconcileResult(
            display: display,
            outcome: .skipped(reason: .missingCurrentProfile, profileID: currentProfileID)))
        continue
      }
      ...
      do {
        ...
        document.displays[recordIndex].lastResult = result.succeeded
          ? nil : ProfileLastResult(summary: result.summary)
        results.append(
          DisplayReconcileResult(
            display: display,
            outcome: .applied(profileID: profile.id, result: result)))
      } catch {
        document.displays[recordIndex].lastResult = ProfileLastResult(
          summary: "failed: \(error)")
        results.append(
          DisplayReconcileResult(
            display: display,
            outcome: .skipped(reason: .failed, profileID: profile.id)))
      }
    }
  ...
  func reconcileIntent(
    _ intent: DisplayProfileIntent,
    for display: DisplaySelector,
    trigger: DisplayEventTrigger
  ) throws -> DisplayProfileIntent {
    guard trigger.isSteadyState, intent.displayMode != nil else {
      return intent
    }
    ...
    guard state.hdrMode.readability == .readable,
      let currentHDR = state.hdrMode.value
    else {
      var guardedIntent = intent
      guardedIntent.displayMode = nil
      return guardedIntent
    }
    guard currentHDR != expectedHDR else {
      return intent
    }
